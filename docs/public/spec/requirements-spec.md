...
- **RF6.1.7 Actualizado -- Publicación continua en Microsoft Store:** La publicación a Microsoft Store se autoriza únicamente a partir de una GitHub Release estable publicada (`published`, no draft ni prerelease). La versión de Store se deriva de forma determinista y monótona del producto canónico mediante la transformación `A.B.C.D → A.B.S.0` con base de empaquetado 256 (revisión de Store siempre cero), y los artefactos enviados a Store son exactamente los bytes validados de la release. El artefacto de distribución de Store es un único `.msixbundle` no firmado que contiene exactamente un paquete `ProcessorArchitecture=x64` y un paquete `ProcessorArchitecture=arm64`, con la misma identidad reservada, publicador, versión, capacidades (`runFullTrust`, `microphone`) y familia de dispositivos (`Windows.Desktop`). El pipeline automatizado consulta el estado remoto de Partner Center antes de cualquier mutación, diferencia preparado/enviado/comprometido/certificando/publicado/fallido, es idempotente ante reejecuciones y estados parciales, nunca elimina una submission ajena, y conserva la política de privacidad en una URL pública sin modificar la metadata de listado (descripciones, capturas, precios o disponibilidad). El mecanismo de envío automatizado no se fija a una herramienta específica: se usa el CLI oficial actual de desarrollo de Store de Microsoft con coordenadas versionadas en `build/Product.props`.
