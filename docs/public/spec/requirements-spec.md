...
- **RF6.1.7 Nuevo -- Publicación continua en Microsoft Store:** Para publicar en Microsoft Store, la versión del paquete debe derivarse del producto canónico con revisión cero (`A.B.C.0`), y cada arquitectura soportada (x64 y ARM64) debe emitirse como un paquete `.msix` individual con su propia `ProcessorArchitecture`, nunca como dos bundles aplicables al mismo conjunto de dispositivos. El pipeline debe permitir enviar actualizaciones de forma automatizada mediante la API de envío (StoreBroker), conservando la identidad reservada, las capacidades (`runFullTrust`, `microphone`), la política de privacidad en una URL pública y versiones de Store estrictamente crecientes.
